...
export default function AdminEditProduct() {
  ...
  const { getProductById, updateProduct, deleteProduct, categories } = useProducts();
  ...
  if (loading) return <div className="flex items-center justify-center min-h-[400px] font-bold text-gray-400 uppercase tracking-widest animate-pulse">Loading Product Data...</div>;
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center space-x-4">
          ...
          <div className="space-y-1">
            <h1 className="text-2xl md:text-3xl font-bold text-[#2D2D2D]">Edit Product</h1>
            <p className="text-sm text-gray-500">Update the details of your store collection.</p>
          </div>
        </div>
        <button 
          type="submit"
      ...
      {/* Delete Action (Moved outside form) */}
      <div className="pt-12 border-t border-gray-100 mt-12 flex justify-center pb-20">
        <div className="bg-red-50 p-10 rounded-[2.5rem] border border-red-100 max-w-2xl w-full text-center space-y-6">
          <div className="space-y-2">
            <h3 className="text-xl font-bold text-red-600 uppercase tracking-widest">Danger Zone</h3>
            <p className="text-gray-500 text-sm">Once you delete a product, it cannot be recovered. Please be careful.</p>
          </div>
          <button 
            type="button"
            onClick={(e) => {
              ...
              if (id && window.confirm(`Are you sure you want to delete "${formData.name}"?`)) {
                deleteProduct(id);
                alert('Product deleted successfully');
                navigate('/admin/products');
              }
            }}
            className="px-10 py-4 bg-white text-red-500 border-2 border-red-200 rounded-2xl font-bold hover:bg-red-500 hover:text-white transition-all hover:border-red-500 shadow-sm cursor-pointer active:scale-95"
          >
            DELETE THIS PRODUCT
          </button>
        </div>
